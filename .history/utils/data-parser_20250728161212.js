...
class DataParserUtils {
    ...
    isLikelyNoteItem(element) {
        // 检查是否包含链接
        const hasLink = element.querySelector('a[href*="/explore/"], a[href*="/note/"]');
    ...
    extractStats(element) {
        ...
        for (const textElement of textElements) {
            ...
            // 根据文本内容判断数据类型
            if (this.patterns.likes.test(text) && stats.likes === null) {
                stats.likes = number;
            } else if (this.patterns.collects.test(text) && stats.collects === null) {
                stats.collects = number;
            } else if (this.patterns.comments.test(text) && stats.comments === null) {
